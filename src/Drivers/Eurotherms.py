...
class Eurotherm2408(AbstractController, minimalmodbus.Instrument):
    ...
    def set_rate(self, rate):
        ...
        with self.com_lock:
            self.write_register(35, rate, number_of_decimals=0)

    def get_rate(self):
        with self.com_lock:
            return self.read_register(35, number_of_decimals=0)
...
class Eurotherm3508(AbstractController, minimalmodbus.Instrument):
    ...
    * _slave_address (int): slave address in the range 1 to 247
    ...
    def set_rate(self, rate):
        """Set the rate of change for the working setpoint i.e., the heating/cooling rate"""
        with self.com_lock:
            self.write_register(35, rate / 1000, number_of_decimals=1, signed=True)
...
class Eurotherm3508S(AbstractSensor, minimalmodbus.Instrument):
    ...
    def __init__(self, _port, _slave_address=1, baudrate=9600):
        super().__init__(_port, _slave_address)
        self.serial.baudrate = baudrate
        self.com_lock = threading.Lock()
